  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
            ...
            mainEntity: {
              ...
              itemListElement: filteredProjects.map((project, index) => ({
                ...
                item: {
                  ...
                  url: `https://lueurstudio.fr/portfolio/${project.slug}`,
                  image: project.image.startsWith("http") ? project.image : `https://lueurstudio.fr${project.image}`,
                },
              })),
            },
          }),
      ...
      <header className="mx-auto w-full max-w-6xl px-4 pb-6 sm:pb-8 md:pb-10 pt-20 sm:px-6 md:px-10 lg:px-14">
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-xs sm:text-sm text-indigo-200 transition hover:text-white mb-4 sm:mb-6"
        >
          <svg
            className="h-4 w-4"
            fill="none"
        ...
        <div className="space-y-4 sm:space-y-6 rounded-3xl border border-white/10 bg-gradient-to-br from-white/10 via-white/5 to-slate-900/70 p-6 sm:p-8 md:p-10 shadow-2xl shadow-indigo-950/30">
          ...
          <div className="flex flex-wrap gap-2 overflow-x-auto pb-2 -mx-2 px-2 sm:mx-0 sm:px-0">
            <button
              ...
              className={`rounded-full px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold transition whitespace-nowrap ${selectedCategory === null
      ...
      <main className="mx-auto flex max-w-6xl flex-col gap-8 sm:gap-12 px-4 sm:px-6 md:px-10 lg:px-14 pb-16 sm:pb-20 md:pb-24">
        <section className="grid gap-6 sm:gap-8 rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-6 md:p-8 shadow-2xl shadow-indigo-950/30 lg:grid-cols-[1.2fr,0.8fr]">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end gap-1 p-4">
                      <span className="w-fit rounded-full bg-white/15 px-2 py-1 text-xs font-semibold uppercase tracking-wide text-indigo-100 ring-1 ring-white/20">
                        {project.category}
                      </span>
          ...
          {activeProject && (
            <div className="flex h-full flex-col gap-3 sm:gap-4 rounded-3xl border border-white/10 bg-slate-900/80 p-4 sm:p-6">
              ...
              <p className="text-xs sm:text-sm uppercase tracking-[0.4em] text-indigo-200">
                {activeProject.subtitle}
              </p>
              <p className="text-sm sm:text-base text-slate-200">{activeProject.description}</p>
              <ul className="space-y-1 sm:space-y-2 text-xs sm:text-sm text-indigo-50">
              ...
              <div className="mt-auto flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 pt-3 sm:pt-4">
                <Link
                  ...
                  className="rounded-full bg-white px-4 py-2 sm:px-5 sm:py-2 text-xs sm:text-sm font-semibold text-slate-900 transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-500/40 text-center"
                >
                  Voir toutes les photos
                </Link>
                <Link
                  href="/#contact"
                  className="rounded-full border border-white/20 px-4 py-2 sm:px-5 sm:py-2 text-xs sm:text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:border-white/40 hover:bg-white/10 text-center"
                >
        ...
        <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 sm:p-8 text-center shadow-xl shadow-black/40">
          ...
          <p className="mt-2 sm:mt-3 text-base sm:text-lg text-slate-200">
            Brief gratuit en visio ou sur place. Je te propose un moodboard personnalisé, un planning détaillé et un devis transparent en moins de 24h. Prêt à démarrer ?
          </p>
          <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row flex-wrap justify-center gap-2 sm:gap-3">
            ...
            <Link
              ...
              className="rounded-full border border-white/20 px-5 py-2.5 sm:px-6 sm:py-3 text-xs sm:text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:border-white/40 hover:bg-white/10 break-all sm:break-normal"
            >
              {CONTACT_EMAIL}
            </Link>
          </div>
